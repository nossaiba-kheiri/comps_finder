...
def view_extraction(ticker):
    ...
    if econ_sig and isinstance(econ_sig, dict):
        # New economic signature fields (preferred)
        if 'capacity_unit' in econ_sig:
            print(f"  Capacity Unit: {econ_sig.get('capacity_unit', 'none')}")
            pricing_basis = econ_sig.get('pricing_basis', [])
            if pricing_basis:
                print(f"  Pricing Basis: {', '.join(pricing_basis)}")
            print(f"  Asset Intensity: {econ_sig.get('asset_intensity_0_1', econ_sig.get('asset_intensity', 0)):.2f} (0=light, 1=heavy)")
            print(f"  Revenue Recurring: {econ_sig.get('revenue_recurring_0_1', 0):.2f} (0=one-time, 1=fully recurring)")
            print(f"  Inventory Fragmentation: {econ_sig.get('inventory_fragmentation_0_1', 0):.2f} (0=unified, 1=highly fragmented)")
            print(f"  Demand Matching Role: {econ_sig.get('demand_matching_role', 'none')}")
            print(f"  Utilization Metric: {econ_sig.get('utilization_metric', 'none')}")
        # Legacy economic signature fields (fallback)
        else:
